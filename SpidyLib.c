...
Bloc **NeedGrid(int rows, int cols, Texture Minerai_commun, Texture Minerai_rare, Texture evenement) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            ...
            if (i < 8) {
                if (randValue < 80) {
                    Grille[i][j].Texture = Minerai_commun;
                    Grille[i][j].type = COMMUN;
                } else if (randValue < 90) {
                    Grille[i][j].Texture = Minerai_rare;
                    Grille[i][j].type = RARE;
                } else {
...
void DrawInventaire(Inventaire *inventaire, int HauteurLigne) {
    DrawText(TextFormat("Minerai commun : %d",inventaire->Mineraie_Niveau_01), 0,HauteurLigne+40 ,20,GREEN );
    DrawText(TextFormat("Minerai rare : %d",inventaire->Mineraie_Niveau_02), 0,HauteurLigne+60 ,20,BLUE );
    DrawText(TextFormat("Minerai épique : %d",inventaire->Mineraie_Niveau_03), 0,HauteurLigne+80 ,20,PURPLE );
}
    ...
    if (IsKeyDown(KEY_SPACE)) {
        *isAction = true;
        ...
        if (IsKeyDown(KEY_RIGHT)) {
            ...
            if (newPosition.x <= ScreenWidth - textures.playerTextureIdle.width && !IsCollidingWithBloc(personnage, Grille, rows, cols)) {
        ...
        if (IsKeyDown(KEY_LEFT)) {
            newPosition.x -= Speed;
            personnage.x = newPosition.x;
            if (newPosition.x >= 0 && !IsCollidingWithBloc(personnage, Grille, rows, cols)) {
                (*playerPosition).x = newPosition.x;
                *isMovingLeft = true;
            }
        }
        if (IsKeyDown(KEY_UP)) {
            ...
            if (newPosition.y >= 0 && !IsCollidingWithBloc(personnage, Grille, rows, cols)) {
        ...
        if (IsKeyDown(KEY_DOWN)) {
            newPosition.y += Speed;
            personnage.y = newPosition.y;
            if (newPosition.y + textures.playerTextureIdle.height <= ScreenHeight && !IsCollidingWithBloc(personnage, Grille, rows, cols)) {
                (*playerPosition).y = newPosition.y;
                *isMovingBas = true;
            }
        }
...
void DrawLimite(int rows, int cols , int offsetX , int startY , int TailleCarre , int Espace , int ScreenWidth , TexturesJeux textures ){
    ...
    for (int i = 0; i < rows; i++) {
        int xLeft = offsetX - (TailleCarre + Espace);
        int yLeft = startY + i * (TailleCarre + Espace);

        DrawTextureEx(textures.incassable, (Vector2){xLeft, yLeft}, 0.0f, (float)TailleCarre / textures.incassable.width, WHITE);
        int xRight = offsetX + cols * (TailleCarre + Espace);
        int yRight = startY + i * (TailleCarre + Espace);
        DrawTextureEx(textures.incassable, (Vector2){xRight, yRight}, 0.0f, (float)TailleCarre / textures.incassable.width, WHITE);
    }
    int yFirstLine = startY;
    for (int x = offsetX-2*(TailleCarre + Espace); x >= 0; x -= (TailleCarre + Espace)) {
        DrawTextureEx(textures.incassable, (Vector2){x, yFirstLine}, 0.0f, (float)TailleCarre / textures.incassable.width, WHITE);
    }
    for (int x = offsetX + cols * (TailleCarre + Espace)+(TailleCarre + Espace); x < ScreenWidth-(TailleCarre+Espace); x += (TailleCarre + Espace)) {
        DrawTextureEx(textures.incassable, (Vector2){x, yFirstLine}, 0.0f, (float)TailleCarre / textures.incassable.width, WHITE);
    }
}